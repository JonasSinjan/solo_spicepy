...
def carrington_observation_times(clon, solo_hdis, ets):
    ...
    #LLD
    #start_time = "3 April 2022 19:24 (UTC)"   # LTP05 during high omega CR2256 start time
    #delta_phi  = 13.198754917452785# 14.184397163120567
    #n_obs      = int(np.ceil(361. / delta_phi))
    #RSW
    #start_time = "11 April 2022 00:00 (UTC)"   # LTP05 RSW North
    #delta_phi  = 2.199792486242131# 2.3640661938534278 # deg4h=(360/(27.2753d*24h))*4h Earth based Carrington rotation necessary for earthlike grid
    #n_obs      = int(np.ceil(150. / delta_phi))
    ...
    # FSM w/ RSW
    start_time = "18 April 2022 03:00 (UTC)"   # 
    delta_phi  = 2.199792486242131
    n_obs      = int(np.ceil(120. / delta_phi))
    
    # 24h = 14.184397163120567 deg, spacing between observations on carrington grid / RSW: 4h = 2.3640661938534278 deg
    ...
    obs_dist = np.append(obs_dist, solo_hdis[t0_index])
    ...
    for i in range(1,n_obs):
        ...
        obs_dist = np.append(obs_dist, np.interp(clon_obs_lin, clon_linear, solo_hdis))
        ...
        #obs_clon = np.append(obs_clon, clon_obs-clon_linear[t0_index]+clon_t0)
        #obs_clon = np.append(obs_clon, clon_obs-clon_linear[t0_index]+clon_t0)
    ...
    print(output)
    return obs_utc, obs_et

def carrington_observation_deg(clon, solo_hdis, ets):
...
def carrington_observation_duration(solo_clon, earth_clon, solo_hdis, ets):
    ...
    plt.plot(ltp, dt)
    plt.title('Synoptic Map Creation Time')
    plt.xlabel('LTP')
    plt.ylabel('Days')
    plt.show()
...
def carrington_observation_coverage(solo_clon, earth_clon, solo_hdis, ets):
...
def main():
    ...
    solo_cobs, solo_cet = carrington_observation_times(solo_clon, solo_hdis, ets)
    solo_cobs, solo_cet, obs_clon = carrington_observation_deg(solo_clon, solo_hdis, ets)

    earth_clon = simple_carrington(earth_hlon,ets)

    dt = carrington_observation_duration(solo_clon, earth_clon, solo_hdis, ets)
    coverage_solo, coverage_earth = carrington_observation_coverage(solo_clon, earth_clon, solo_hdis, ets)
